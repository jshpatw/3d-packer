...
@njit
def get_total_overlap_v60(pos, qs, n):
...
@njit
def run_monte_carlo_v60(pos, qs, n, steps=20000, temp=0.01):
    current_overlap = get_total_overlap_v60(pos, qs, n)
    for s in range(steps):
        ...
        new_overlap = get_total_overlap_v60(pos, qs, n)
        if new_overlap < current_overlap:
            current_overlap = new_overlap
        elif np.random.rand() < np.exp(-(new_overlap - current_overlap) / (temp * 0.2)):
            current_overlap = new_overlap
        else:
...
def run_v60_hyper_density(target_n=24):
    print(f"Starting SAGE V60 (Hyper-Density, {target_n} units)...")
    ...
    for layer in range(2):
        for row in range(2):
            for col in range(6):
                ...
                positions[idx] = [0.6 + col * 0.94, 0.6 + row * 1.15, 0.6 + layer * 1.15]
                if (col + row + layer) % 2 == 1:
                    angle = np.pi / 3 
    ...
    total_steps = 400000
    curr_ov = get_total_overlap_v60(positions, quaternions, target_n)
    print(f"  Initial Overlap: {curr_ov:.4f}m")
    
    # Warmup
    print("  Warmup...")
    _ = run_monte_carlo_v60(positions, quaternions, target_n, steps=100, temp=0.1)
    
    for block in range(total_steps // steps_per_block):
        progress = (block*steps_per_block)/total_steps
        temp = 0.1 * (1.0 - progress)**2 + 0.001
        curr_ov, s = run_monte_carlo_v60(positions, quaternions, target_n, steps=steps_per_block, temp=temp)
        if block % 10 == 0:
...
if __name__ == "__main__":
    try:
        target = 24
        pos, qs, count, ov = run_v60_hyper_density(target_n=target)
        ...
        output_path = f"sage_tetrapod/export_results/60_HyperDensity_{count}Unit.glb"
        scene.export(output_path)
        print(f"V60 Result saved to {output_path}")

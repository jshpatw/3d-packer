...
@njit
def resolve_capacity(positions, quaternions, n, current_scale, iterations=50):
            ...
            for j in range(i + 1, n):
                ...
                for k1 in range(4):
                    ...
                    for k2 in range(4):
                        ...
                        target = (rb + t*(rt-rb)) + (rb + u*(rt-rb)) + 0.002
                        if d_sq < target**2:
                            dist = np.sqrt(d_sq)
                            overlap = target - dist
...
def run_capacity_probe(target_n=22):
    print(f"Starting Capacity Probe ({target_n} units)...")
    ...
    while current_scale < 1.0:
        current_scale += 0.005
        overlap = resolve_capacity(positions, quaternions, target_n, current_scale, iterations=60)
        if overlap > 0.05: # If overlap exceeds 5cm, we are hitting the limit
            print(f"  CRITICAL JAM at Scale {current_scale:.3f} | Overlap: {overlap:.4f}m")
            current_scale -= 0.005
            break
            
    print(f"  Stable Capacity Limit for {target_n} units: Scale {current_scale:.3f}")
    return positions, quaternions, target_n, current_scale

if __name__ == "__main__":
    try:
        pos, qs, count, scale = run_capacity_probe(target_n=22)
        import generate_mesh
        # Generate mesh at the ACHEVED scale to show it's collision-free
        tp_mesh = generate_mesh.create_tetrapod(h=1.13 * scale)
        scene = trimesh.Scene()
        ...
        output_path = "sage_tetrapod/export_results/51_CapacityProbe.glb"

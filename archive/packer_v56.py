...
@njit
def get_total_overlap_v56(pos, qs, n):
...
@njit
def run_monte_carlo_frontier(pos, qs, n, steps=1000000):
    current_overlap = get_total_overlap_v56(pos, qs, n)
...
def run_v56_frontier(target_n=23):
    print(f"Starting SAGE V56 (Frontier Push, {target_n} units)...")
        ...
        for layer in range(2):
            for row in range(2):
                for col in range(6):
                    ...
                    # More precise starting grid
                    x = 0.5 + col * 0.95
                    y = 0.6 + row * 1.15
                    z = 0.6 + layer * 1.1
                    positions[idx] = [x, y, z]
                    # Up-down orientation strategy
                    if layer == 1:
                        quaternions[idx] = np.array([0.0, 0.0, 1.0, 0.0])
                    else:
                        quaternions[idx] = np.array([1.0, 0.0, 0.0, 0.0])
                    idx += 1

        # Manual sub-loop to prevent timeout
        steps_per_block = 50000
        total_steps = 300000
        curr_ov = 999.0
        for block in range(total_steps // steps_per_block):
            curr_ov, s = run_monte_carlo_frontier(positions, quaternions, target_n, steps=steps_per_block)
            print(f"    Block {block+1} | Overlap: {curr_ov:.4f}m")
            if curr_ov < 1e-6: break

        if curr_ov < best_ov:
            best_ov = curr_ov; best_pos = positions.copy(); best_qs = quaternions.copy()
        if best_ov < 1e-6: break

    print(f"  Final Best Overlap for {target_n} units: {best_ov:.6f}m")
...
if __name__ == "__main__":
    try:
        # Pushing to 24 units
        target = 24
        pos, qs, count, ov = run_v56_frontier(target_n=target)
        ...
        output_path = f"sage_tetrapod/export_results/56_Frontier_{count}Unit.glb"
        scene.export(output_path)
        print(f"V56 Result saved to {output_path}")

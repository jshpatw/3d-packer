...
@njit
def get_total_overlap_v57(pos, qs, n):
...
@njit
def run_monte_carlo_v57(pos, qs, n, steps=1000000, start_temp=0.1, cool_factor=3):
    current_overlap = get_total_overlap_v57(pos, qs, n)
    for s in range(steps):
        i = np.random.randint(0, n)
        old_p = pos[i].copy(); old_q = qs[i].copy()
        temp = start_temp * (1.0 - s/steps)**cool_factor + 0.001
        pos[i] += (np.random.rand(3) - 0.5) * temp
        dq = (np.random.rand(4) - 0.5) * temp * 5.0
        qs[i] = (qs[i] + dq) / np.linalg.norm(qs[i] + dq)
        new_overlap = get_total_overlap_v57(pos, qs, n)
        if new_overlap < current_overlap:
            current_overlap = new_overlap
        elif np.random.rand() < np.exp(-(new_overlap - current_overlap) / (temp * 0.5)):
            current_overlap = new_overlap
        else:
...
def run_v57_frontier(target_n=23):
    print(f"Starting SAGE V57 (Frontier Push, {target_n} units)...")
    # Trigger JIT
    print("  Pre-compiling kernels...")
    _ = get_total_overlap_v57(np.random.rand(target_n, 3), np.random.rand(target_n, 4), target_n)
    ...
    for layer in range(2):
        for row in range(2):
            units_per_row = target_n // 4 + (1 if (idx < target_n % 4) else 0)
            for col in range(6):
                if idx >= target_n: break
                positions[idx] = [0.6 + col * 0.9, 0.6 + row * 1.1, 0.6 + layer * 1.15]
                quaternions[idx] = np.array([1.0, 0, 0, 0]) if (col+row)%2==0 else np.array([0, 0, 1.0, 0])
                idx += 1

    steps_per_block = 10000
    total_steps = 500000
    curr_ov = get_total_overlap_v57(positions, quaternions, target_n)
    print(f"  Initial Overlap: {curr_ov:.4f}m")
    
    for block in range(total_steps // steps_per_block):
        curr_ov, s = run_monte_carlo_v57(positions, quaternions, target_n, steps=steps_per_block)
        print(f"    Block {block+1:3d} | Total Overlap: {curr_ov:.6f}m")
...
if __name__ == "__main__":
    try:
        # Pushing to 23 units first
        target = 23
        pos, qs, count, ov = run_v57_frontier(target_n=target)
        ...
        output_path = f"sage_tetrapod/export_results/57_Frontier_{count}Unit.glb"
        scene.export(output_path)
        print(f"V57 Result saved to {output_path} | Overlap: {ov:.6f}m")

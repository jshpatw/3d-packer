...
@njit
def get_total_overlap_v63(pos, qs, n):
    ...
    for i in range(n):
        for d in range(3):
            if pos[i, d] < R_BASE: total += (R_BASE - pos[i, d]) * 20.0
            if pos[i, d] > REAL_CONTAINER[d] - R_BASE: total += (pos[i, d] - (REAL_CONTAINER[d] - R_BASE)) * 20.0
        for k in range(4):
            tip = pos[i] + all_dirs[i, k] * LEG_H
            for d in range(3):
                if tip[d] < R_TIP: total += (R_TIP - tip[d]) * 20.0
                if tip[d] > REAL_CONTAINER[d] - R_TIP: total += (tip[d] - (REAL_CONTAINER[d] - R_TIP)) * 20.0
...
@njit
def run_monte_carlo_v63(pos, qs, n, steps=100000, temp=0.01):
    current_overlap = get_total_overlap_v63(pos, qs, n)
    for s in range(steps):
        ...
        new_overlap = get_total_overlap_v63(pos, qs, n)
...
def run_v63_ultra_push(target_n=23):
    print(f"Starting SAGE V63 (Ultra Deep Push, {target_n} units)...")
    positions = np.zeros((target_n, 3))
    quaternions = np.zeros((target_n, 4))
    
    # ADVANCED INITIALIZATION: TIGHT HEXADECIMAL
    idx = 0
    for layer in range(2):
        for row in range(2):
            for col in range(6):
                if idx >= target_n: break
                positions[idx] = [0.6 + col * 0.93, 0.6 + row * 1.15, 0.6 + layer * 1.15]
                angle = (np.pi / 3) if (col + row + layer) % 2 == 1 else 0
                quaternions[idx] = [np.cos(angle/2), 0, 0, np.sin(angle/2)]
                idx += 1

    steps_per_block = 50000
    total_steps = 1000000 # 1M steps for a definitive answer
    curr_ov = get_total_overlap_v63(positions, quaternions, target_n)
    ...
    for block in range(total_steps // steps_per_block):
        ...
        temp = 0.15 * (1.0 - progress)**2 + 0.0005
        curr_ov, s = run_monte_carlo_v63(positions, quaternions, target_n, steps=steps_per_block, temp=temp)
        print(f"    Step {block*steps_per_block:7d} | Overlap: {curr_ov:.6f}m | T: {temp:.4f}")
...
if __name__ == "__main__":
    try:
        target = 23
        pos, qs, count, ov = run_v63_ultra_push(target_n=target)
        ...
        output_path = f"sage_tetrapod/export_results/04_Frontier_Research/63_UltraPush_{count}Unit.glb"
        scene.export(output_path)
        print(f"V63 Result saved to {output_path} | Final Overlap: {ov:.6f}m")

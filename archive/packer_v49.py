...
@njit
def smart_resolve_step(pos, q, packed_pos, packed_dirs, num_packed):
        ...
        for k1 in range(4):
            ...
            for k2 in range(4):
                ...
                if d_sq < target**2:
                    ...
                    dq = 0.5 * q_multiply(q, np.array([0.0, torque[0], torque[1], torque[2]])) * 0.8
                    q += dq; q /= np.linalg.norm(q)
                    for k in range(4): my_dirs[k] = q_rotate(q, orig_dirs[k])

    # 2. HARD CONTAINER PUSH
    for d in range(3):
...
@njit
def final_verify(pos, q, packed_pos, packed_dirs, num_packed):
    orig_dirs = np.array([[0.0, 0.0, 1.0], [0.9428, 0.0, -0.3333], [-0.4714, 0.8165, -0.3333], [-0.4714, -0.8165, -0.3333]])
    my_dirs = np.zeros((4, 3))
    for k in range(4): my_dirs[k] = q_rotate(q, orig_dirs[k])
...
def run_v49_smart_stacker(target_n=22):
    print(f"Starting SAGE V49 (PBD Smart Stacker, {target_n} units)...")
    ...
    for i in range(target_n):
        ...
        for attempt in range(80):
            ...
            # Smart Physics Settlement
            for step in range(3500):
                pos[2] -= 0.015 # Heavy gravity
                pos, q = smart_resolve_step(pos, q, packed_pos, packed_dirs, num_packed)
                if step < 2500: pos += (np.random.rand(3) - 0.5) * 0.005 # Wiggle
            
            # Verify
            if final_verify(pos, q, packed_pos, packed_dirs, num_packed):
                packed_pos[num_packed] = pos
                packed_qs[num_packed] = q
                for k in range(4): packed_dirs[num_packed, k] = q_rotate(q, orig_dirs[k])
                num_packed += 1
                print(f"  Unit {num_packed}/{target_n} SECURED at Z={pos[2]:.2f} (Attempt {attempt+1})")
                success = True
                break
        
        if not success:
            print(f"  Stopping at {num_packed} units. Container density limit reached.")
            break
...
if __name__ == "__main__":
    try:
        pos, qs, count = run_v49_smart_stacker(target_n=22)
        ...
        output_path = "sage_tetrapod/export_results/49_IronContainer_V49_Smart.glb"
        scene.export(output_path)
        print(f"V49 Result saved to {output_path} | Count: {count}")

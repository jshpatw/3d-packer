...
@njit
def sequential_grow_step(pos, q, packed_pos, packed_dirs, num_packed, current_scale, iterations=10):
    orig_dirs = np.array([[0.0, 0.0, 1.0], [0.9428, 0.0, -0.3333], [-0.4714, 0.8165, -0.3333], [-0.4714, -0.8165, -0.3333]])
    rb_me = R_BASE * current_scale
    rt_me = R_TIP * current_scale
        ...
        for i in range(num_packed):
            ...
            for k1 in range(4):
                ...
                for k2 in range(4):
                    ...
                    target = (rb_me + t*(rt_me-rb_me)) + (R_BASE + u*(R_TIP-R_BASE)) + 0.01
                    if d_sq < target**2:
                        dist = np.sqrt(d_sq)
                        ...
                        dq = 0.5 * q_multiply(q, np.array([0.0, torque[0], torque[1], torque[2]])) * 1.5
                        q += dq; q /= np.linalg.norm(q)
                        for k in range(4): my_dirs[k] = q_rotate(q, orig_dirs[k])
        # 2. Container
        for d in range(3):
            if pos[d] < rb_me + 0.01: pos[d] = rb_me + 0.01
            if pos[d] > REAL_CONTAINER[d] - rb_me - 0.01: pos[d] = REAL_CONTAINER[d] - rb_me - 0.01
        for k in range(4):
            point = pos + my_dirs[k] * LEG_H
            rad = rt_me + 0.01
            for d in range(3):
                if point[d] < rad: pos[d] += (rad - point[d])
                if point[d] > REAL_CONTAINER[d] - rad: pos[d] -= (point[d] - (REAL_CONTAINER[d] - rad))
    return pos, q

@njit
def verify_strict(pos, q, packed_pos, packed_dirs, num_packed):
    orig_dirs = np.array([[0.0, 0.0, 1.0], [0.9428, 0.0, -0.3333], [-0.4714, 0.8165, -0.3333], [-0.4714, -0.8165, -0.3333]])
    my_dirs = np.zeros((4, 3))
    for k in range(4): my_dirs[k] = q_rotate(q, orig_dirs[k])
    for i in range(num_packed):
        ...
        for k1 in range(4):
            ...
            for k2 in range(4):
                ...
                if d_sq < (target - 0.001)**2: return False
    return True

def run_v53_growth_sequential(target_n=22):
    print(f"Starting SAGE V53 (Sequential Growth Stacker, {target_n} units)...")
    ...
    for i in range(target_n):
        ...
        for attempt in range(60):
            pos = np.array([
                ...
                R_BASE + 0.1 + np.random.rand() * (REAL_CONTAINER[2] - 2*R_BASE - 0.2)
            ])
            q = np.random.rand(4) - 0.5; q /= np.linalg.norm(q)
            
            # Growth Phase
            for grow_step in range(1500):
                scale = 0.1 + 0.9 * (grow_step / 1500.0)
                pos, q = sequential_grow_step(pos, q, packed_pos, packed_dirs, num_packed, scale, iterations=2)
                if grow_step < 1000:
                    pos += (np.random.rand(3) - 0.5) * 0.005
            
            if verify_strict(pos, q, packed_pos, packed_dirs, num_packed):
                packed_pos[num_packed] = pos
                packed_qs[num_packed] = q
                for k in range(4): packed_dirs[num_packed, k] = q_rotate(q, orig_dirs[k])
                num_packed += 1
                print(f"  Unit {num_packed}/{target_n} GROWN at Z={pos[2]:.2f} (Attempt {attempt+1})")
                success = True
                break
        
        if not success:
            print(f"  Sequential growth failed for unit {i+1}. Stopping.")
            break
...
if __name__ == "__main__":
    try:
        pos, qs, count = run_v53_growth_sequential(target_n=22)
        ...
        output_path = "sage_tetrapod/export_results/53_SequentialGrowth.glb"
        scene.export(output_path)
        print(f"V53 Result saved to {output_path} | Count: {count}")

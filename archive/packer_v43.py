...
@njit
def check_collision_boolean(pos, dirs, packed_pos, packed_dirs, num_packed, buffer=0.005):
        ...
        for k1 in range(4):
            ...
            for k2 in range(4):
                ...
                if d_sq < target**2:
                    ...
                    dq = 0.5 * q_multiply(q, np.array([0.0, torque[0], torque[1], torque[2]])) * 0.4
                    q += dq; q /= np.linalg.norm(q)
                    for k in range(4): my_dirs[k] = q_rotate(q, orig_dirs[k])

    # 2. Container
    for d in range(3):
        if pos[d] < R_BASE + 0.01: pos[d] = R_BASE + 0.01
        if pos[d] > REAL_CONTAINER[d] - R_BASE - 0.01: pos[d] = REAL_CONTAINER[d] - R_BASE - 0.01
    for k in range(4):
...
def run_zero_tolerance_packer(target_n=22):
    print(f"Starting SAGE V43 (Zero-Tolerance, {target_n} units)...")
    ...
    for i in range(target_n):
        success = False
        for attempt in range(40): # More attempts for tight fit
            # Start at top
            pos = np.array([
            ...
            for step in range(2500):
                ...
                if step < 1500: pos += (np.random.rand(3) - 0.5) * 0.005 # Shaking
            
            # FINAL VERIFICATION
            my_dirs = np.zeros((4, 3))
            for k in range(4): my_dirs[k] = q_rotate(q, orig_dirs[k])
            
            if not check_collision_boolean(pos, my_dirs, packed_pos, packed_dirs, num_packed, buffer=0.002):
                packed_pos[num_packed] = pos
                packed_qs[num_packed] = q
                packed_dirs[num_packed] = my_dirs
                num_packed += 1
                print(f"  Unit {num_packed}/{target_n} GUARANTEED at Z={pos[2]:.2f} (Attempt {attempt+1})")
                success = True
                break
        
        if not success:
            print(f"  FAILED to find valid spot for unit {i+1}. Container may be full.")
            break
...
if __name__ == "__main__":
    try:
        pos, qs, count = run_zero_tolerance_packer(target_n=22)
        ...
        output_path = "sage_tetrapod/export_results/43_IronContainer_V43_Guaranteed.glb"
        scene.export(output_path)
        print(f"V43 Result saved to {output_path} | Valid Units: {count}")

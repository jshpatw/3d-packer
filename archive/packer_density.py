...
@njit
def is_outside_strict(pos, dirs, buffer=0.005):
    for d in range(3):
...
@njit
def check_collision(pos1, dirs1, pos2, dirs2, buffer=0.01):
...
@njit
def count_all_collisions(positions, all_dirs, n, buffer=0.01):
    count = 0
    for i in range(n):
        for j in range(i + 1, n):
            if check_collision(positions[i], all_dirs[i], positions[j], all_dirs[j], buffer):
                count += 1
    return count

@njit
def run_mc_relaxation(positions, matrices, n, iterations=2000000):
    all_dirs = np.zeros((n, 4, 3))
    for i in range(n):
        all_dirs[i] = get_dirs(matrices[i])
    current_collisions = count_all_collisions(positions, all_dirs, n)
    for step in range(iterations):
        progress = step / iterations
        idx = np.random.randint(n)
        old_pos, old_mat, old_dirs = positions[idx].copy(), matrices[idx].copy(), all_dirs[idx].copy()
        scale = 0.12 * (1.0 - progress)**2.5 + 0.002
        move = (np.random.rand(3) - 0.5) * scale
        move[2] -= 0.005 * (1.0 - progress)
        rot_axis = np.random.rand(3) - 0.5; rot_axis /= (np.linalg.norm(rot_axis) + 1e-9)
        a = (np.random.rand() - 0.5) * scale * 2.0
        ux, uy, uz = rot_axis
        ...
        if is_outside_strict(new_pos, new_dirs): continue
        old_hits, new_hits = 0, 0
        for j in range(n):
            if idx == j: continue
            if check_collision(old_pos, old_dirs, positions[j], all_dirs[j]): old_hits += 1
            if check_collision(new_pos, new_dirs, positions[j], all_dirs[j]): new_hits += 1
        if new_hits < old_hits:
            ...
            if new_pos[2] < old_pos[2]:
                positions[idx], matrices[idx], all_dirs[idx] = new_pos, new_mat, new_dirs
        if current_collisions == 0 and progress > 0.95: break
    return current_collisions

def run_density_optimizer(start_count=22):
    print(f"Starting Extreme SAGE Optimizer (Target: {start_count} units)...")
    positions = np.random.rand(start_count, 3)
    positions[:, 0] = positions[:, 0] * (REAL_CONTAINER[0]-1.2) + 0.6
    positions[:, 1] = positions[:, 1] * (REAL_CONTAINER[1]-1.2) + 0.6
    positions[:, 2] = positions[:, 2] * (REAL_CONTAINER[2]-1.2) + 0.6
    matrices = np.zeros((start_count, 3, 3))
    for i in range(start_count):
        matrices[i] = trimesh.transformations.random_rotation_matrix()[:3, :3]
        while is_outside_strict(positions[i], get_dirs(matrices[i])):
            positions[i] = np.random.rand(3) * (REAL_CONTAINER - 1.2) + 0.6
    curr_n = start_count
    while curr_n > 0:
        print(f"--- ATTEMPTING {curr_n} UNITS ---")
        collisions = run_mc_relaxation(positions[:curr_n], matrices[:curr_n], curr_n)
        print(f"Result: {collisions} collisions.")
        if collisions == 0: break
            ...
            for i in range(curr_n):
                for j in range(i + 1, curr_n):
                    if check_collision(positions[i], all_dirs[i], positions[j], all_dirs[j]):
                        hits[i] += 1; hits[j] += 1
            victim = np.argmax(hits)
            print(f"Culling unit {victim}..."); positions[victim:curr_n-1] = positions[victim+1:curr_n]; matrices[victim:curr_n-1] = matrices[victim+1:curr_n]; curr_n -= 1
    return positions[:curr_n], matrices[:curr_n], curr_n

if __name__ == "__main__":
    pos, mats, count = run_density_optimizer(start_count=22)
    ...
    output_path = "sage_tetrapod/export_results/27_ExtremeDensity_Final.glb"
    scene.export(output_path); print(f"Final Count: {count}. Result saved to {output_path}")

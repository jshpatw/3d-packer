...
@njit
def enforce_constraints_v38(positions, quaternions, n, current_scale, iterations=50):
    ...
    for _it in range(iterations):
        # Update ALL dirs at start of sub-iteration
        for i in range(n):
            ...
            for j in range(i + 1, n):
                ...
                for k1 in range(4):
                    ...
                    for k2 in range(4):
                        ...
                        r1 = rb + t * (rt - rb)
                        r2 = rb + u * (rt - rb)
                        target = r1 + r2 + 0.015 # 1.5cm comfort gap
                        ...
                        if d_sq < target**2:
                            ...
                            dq_i = 0.5 * q_multiply(quaternions[i], np.array([0.0, torque_i[0], torque_i[1], torque_i[2]])) * 0.8
                            quaternions[i] += dq_i
                            quaternions[i] /= np.linalg.norm(quaternions[i])
                            
                            lever_j = (q1 + u * (q2 - q1)) - positions[j]
                            torque_j = cross(lever_j, -push_vec)
                            dq_j = 0.5 * q_multiply(quaternions[j], np.array([0.0, torque_j[0], torque_j[1], torque_j[2]])) * 0.8
                            quaternions[j] += dq_j
        ...
        for i in range(n):
            ...
            # Leg Tips (Check 5 points per leg)
            for k in range(4):
...
def run_iron_stacker_v38(target_n=22, iterations=5000):
    print(f"Starting SAGE V38 (Precision Solve, {target_n} units)...")
    ...
    for step in range(iterations):
        ...
        for i in range(target_n):
            ...
            # Shaking
            if progress < 0.8:
                shake = 0.04 * (1.0 - progress)
                positions[i] += (np.random.rand(3) - 0.5) * shake
                dq = (np.random.rand(4) - 0.5) * shake * 5.0
                quaternions[i] = (quaternions[i] + dq) / np.linalg.norm(quaternions[i] + dq)
        
        # High-power solver
        solve_iters = 15 if progress < 0.5 else 50
        enforce_constraints_v38(positions, quaternions, target_n, 1.0, iterations=solve_iters)
        
        if step % 1000 == 0:
            print(f"Step {step:5d} | Cooling: {100-progress*100:.1f}%")

    return positions, quaternions, target_n

if __name__ == "__main__":
    try:
        # Reduced iterations to 5000
        pos, qs, count = run_iron_stacker_v38(target_n=22, iterations=5000)
        ...
        container_box.visual.face_colors = [100, 100, 100, 20]
        scene.add_geometry(container_box, node_name="container")
        
        output_path = "sage_tetrapod/export_results/38_IronContainer_V38_Elite.glb"
        scene.export(output_path)
        print(f"V38 Result saved to {output_path}")

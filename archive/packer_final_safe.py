...
@njit
def hard_resolve_final(positions, quaternions, n, iterations=1000):
    ...
    for _it in range(iterations):
        ...
        if max_overlap < 0.0001: break
    return max_overlap

def run_final_safe_pack():
    target_n = 16
    print(f"Starting Final SAGE Pack (16 units, Diagonal Stagger)...")
    ...
    for layer in range(2):
        for row in range(2):
            # 0.7m offset between rows
            x_offset = 0.7 if row == 1 else 0.0
            for col in range(4):
                positions[idx] = [0.8 + col * 1.35 + x_offset, 0.65 + row * 1.1, 0.7 + layer * 1.1]
                q = np.array([1.0, 0, 0, 0])
                quaternions[idx] = q
                idx += 1

    print("  Resolving Geometry (Post-Stagger)...")
    final_overlap = hard_resolve_final(positions, quaternions, target_n, iterations=1000)
...
if __name__ == "__main__":
    try:
        pos, qs, count = run_final_safe_pack()
        ...
        output_path = "sage_tetrapod/export_results/Final_Safe_Pack_18Unit.glb"

...
def get_total_overlap(pos, qs, n):
    ...
    for i in range(n):
        # Container check
        for d in range(3):
            if pos[i, d] < R_BASE: total += (R_BASE - pos[i, d])
            if pos[i, d] > REAL_CONTAINER[d] - R_BASE: total += (pos[i, d] - (REAL_CONTAINER[d] - R_BASE))
        for k in range(4):
            tip = pos[i] + all_dirs[i, k] * LEG_H
            for d in range(3):
                if tip[d] < R_TIP: total += (R_TIP - tip[d])
                if tip[d] > REAL_CONTAINER[d] - R_TIP: total += (tip[d] - (REAL_CONTAINER[d] - R_TIP))
        
        # Inter-object check
        for j in range(i + 1, n):
            if np.sum((pos[i] - pos[j])**2) > (LEG_H * 2.8)**2: continue
            for k1 in range(4):
...
@njit
def run_monte_carlo_resolve(pos, qs, n, steps=20000):
    current_overlap = get_total_overlap(pos, qs, n)
    print("  Initial Overlap:", current_overlap)
    ...
    for s in range(steps):
        ...
        if current_overlap < 1e-6:
            print("  PERFECT ZERO REACHED at step", s)
            break
            
    return current_overlap

def run_v54_monte_carlo(target_n=20):
    print(f"Starting SAGE V54 (Structured Monte Carlo, {target_n} units)...")
    positions = np.zeros((target_n, 3))
    quaternions = np.zeros((target_n, 4))
    
    # Structured Tiling for 20 units (10 per layer, 5x2 grid)
    idx = 0
    for layer in range(2):
        for row in range(2):
            for col in range(5):
                if idx >= target_n: break
                # Highly staggered starting points
                x = 0.7 + col * 1.1 + (0.5 if row == 1 else 0)
                y = 0.6 + row * 1.1
                z = 0.6 + layer * 1.15
                positions[idx] = [x, y, z]
    ...
    final_ov = run_monte_carlo_resolve(positions, quaternions, target_n, steps=200000)
...
if __name__ == "__main__":
    try:
        # Final push to 20 units
        pos, qs, count = run_v54_monte_carlo(target_n=20)
        ...
        output_path = "sage_tetrapod/export_results/54_MonteCarlo_20Unit.glb"
        scene.export(output_path)
        print(f"V54 Result saved to {output_path}")

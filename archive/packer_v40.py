...
@njit
def enforce_constraints_v40(positions, quaternions, n, current_scale, iterations=100):
    ...
    for _it in range(iterations):
                ...
                for k1 in range(4):
                    ...
                    for k2 in range(4):
                        ...
                        if d_sq < target**2:
                            ...
                            dq_i = 0.5 * q_multiply(quaternions[i], np.array([0.0, torque_i[0], torque_i[1], torque_i[2]])) * 2.0
                            quaternions[i] += dq_i
                            ...
                            dq_j = 0.5 * q_multiply(quaternions[j], np.array([0.0, torque_j[0], torque_j[1], torque_j[2]])) * 2.0
                            quaternions[j] += dq_j
        ...
        for i in range(n):
            for d in range(3):
                if positions[i, d] < rb_vis + 0.02: positions[i, d] = rb_vis + 0.02
                if positions[i, d] > REAL_CONTAINER[d] - rb_vis - 0.02: positions[i, d] = REAL_CONTAINER[d] - rb_vis - 0.02
            for k in range(4):
...
def run_iron_stacker_v40(target_n=22, total_steps=8000):
    print(f"Starting SAGE V40 (Extreme Angular Resolve, {target_n} units)...")
    ...
    for step in range(total_steps):
        ...
        for i in range(target_n):
            # Slow gravity
            if progress > 0.3: positions[i, 2] -= 0.012 * (1.0 - progress)
            # Center squeeze
            positions[i, 0:2] += (REAL_CONTAINER[0:2]/2 - positions[i, 0:2]) * 0.0002 * (1.0 - progress)
            # Final cooling shake
            if progress < 0.95:
                shake = 0.06 * (1.0 - progress)
                positions[i] += (np.random.rand(3) - 0.5) * shake
                dq = (np.random.rand(4) - 0.5) * shake * 10.0
                quaternions[i] = (quaternions[i] + dq) / np.linalg.norm(quaternions[i] + dq)
        
        # Solver with ramping iterations
        solve_iters = 10 if progress < 0.5 else 40
        enforce_constraints_v40(positions, quaternions, target_n, current_scale, iterations=solve_iters)
        
        if step % 2000 == 0:
            print(f"Step {step:5d} | Scale: {current_scale:.2f} | Cooling...")

    return positions, quaternions, target_n

if __name__ == "__main__":
    try:
        # 8000 steps with optimized solver
        pos, qs, count = run_iron_stacker_v40(target_n=22, total_steps=8000)
        ...
        output_path = "sage_tetrapod/export_results/40_IronContainer_V40_Extreme.glb"
        scene.export(output_path)
        print(f"V40 Result saved to {output_path}")
